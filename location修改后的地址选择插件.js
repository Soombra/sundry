...
function getAreaListcallback(r){
	...
	if (currentAreaInfo.currentLevel >= 2){
		currentDom.find("a").click(function(e){
			if(page_load){
				page_load = false;
			}
...
(function(){
	...
	//设置点击页面其他位置时，日期面板隐藏。原理不清楚，根据源码调整出来的
	$("body").click(function(){ 
			getStockOpt(currentAreaInfo.currentAreaId,currentAreaInfo.currentAreaName);
	})
	...
	CurrentAreaInfoInit();
	areaTabContainer.eq(0).find("a").click(function(e){
	...
	areaTabContainer.eq(1).find("a").click(function(e){
		areaTabContainer.removeClass("curr");
	...
	areaTabContainer.eq(2).find("a").click(function(e){
		areaTabContainer.removeClass("curr");
	...
	provinceContainer.find("a").click(function(e) {
		if(page_load){
			page_load = false;
		}
		$("#store-selector").unbind("mouseout");
